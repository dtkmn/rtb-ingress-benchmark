...
- benchmark date
- git commit SHA
- git dirty state; published snapshots should come from a clean worktree
- Quarkus platform version when Quarkus lanes are included
- hardware and OS
...
- Do not promote a result captured from a dirty worktree. Dirty runs are useful for upgrade reconnaissance, not published baselines.
...
The runner records the Quarkus platform version and git dirty state in `run-meta.txt`;
the collator carries that into `summary.md` and `summary.json`. Published site refreshes
reject explicit dirty snapshots. Legacy result summaries that predate `git_dirty` fall
back to the committed `site-data.json` snapshot when available; use
`BENCHMARK_SITE_ALLOW_LEGACY_UNKNOWN_DIRTY=1` only when intentionally preserving a
pre-metadata published result without that fallback. Do not use
`BENCHMARK_SITE_ALLOW_DIRTY_PUBLISH=1` for legacy unknown metadata.
